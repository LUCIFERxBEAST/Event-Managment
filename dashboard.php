...
$hosting_query = "SELECT * FROM hackathons WHERE created_by = $user_id ORDER BY event_start ASC";
$hosting = $conn->query($hosting_query);
...
if (!empty($user_skills)) {
    foreach($user_skills as $skill) {
        $clean_skill = $conn->real_escape_string(trim($skill));
        if(!empty($clean_skill)) $skill_filters[] = "event_tags LIKE '%$clean_skill%'";
    }
}
...
<head>
    ...
    <style>
        ...
        /* UI Elements */
        .nav-pills .nav-link { border-radius: 20px; padding: 10px 20px; color: #555; }
...
<nav class="navbar navbar-expand-lg navbar-dark bg-primary shadow-sm">
    <div class="container">
        ...
        <div class="d-flex align-items-center">
            <span class="text-white me-2 d-none d-sm-block">Hi, <?php echo htmlspecialchars($user_name); ?></span>
...
<div class="container main-content my-4">
    ...
    <ul class="nav nav-pills mb-4 justify-content-center" id="pills-tab" role="tablist">
        <li class="nav-item"><button class="nav-link active" data-bs-toggle="pill" data-bs-target="#attending">🎫 My Tickets</button></li>
        <li class="nav-item"><button class="nav-link" data-bs-toggle="pill" data-bs-target="#hosting">🎤 Hosting</button></li>
        <li class="nav-item"><button class="nav-link" data-bs-toggle="pill" data-bs-target="#explore">🌍 Explore</button></li>
    </ul>
    ...
    <div class="tab-content">
        ...
        <div class="tab-pane fade show active" id="attending">
            ...
            <div class="row g-3">
                <?php if ($attending && $attending->num_rows > 0): ?>
                    <?php while($row = $attending->fetch_assoc()): ?>
                    <?php 
                    ...
                    <div class="col-md-4">
                        <div class="card shadow-sm p-3 h-100">
                            <div class="d-flex justify-content-between align-items-start">
                                ...
                                <span class="badge <?php echo $is_present ? 'status-present' : 'status-registered'; ?>"><?php echo $row['status']; ?></span>
                            </div>
                            <p class="text-muted small mb-3">📍 <?php echo htmlspecialchars($row['venue']); ?></p>
                            <div class="mt-auto d-grid gap-2">
                                <a href="ticket.php?hash=<?php echo $row['qr_code_hash']; ?>" class="btn btn-primary btn-sm">View QR Ticket</a>
                                <?php if ($is_present && $is_ended): ?>
                                    <a href="generate_certificate.php?event_id=<?php echo $row['id']; ?>" class="btn btn-success btn-sm fw-bold">🎓 Download Certificate</a>
                                <?php elseif ($is_present && !$is_ended): ?>
                                    <button class="btn btn-warning btn-sm text-dark" disabled>⏳ Cert. available after event</button>
                                <?php else: ?>
                    ...
                    <?php endwhile; ?>
                <?php else: ?>
                    <div class="text-center py-5"><p class="text-muted">No events joined yet.</p></div>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="tab-pane fade" id="hosting">
            ...
            <div class="row g-3">
                <?php if ($hosting && $hosting->num_rows > 0): ?>
                    <?php while($row = $hosting->fetch_assoc()): ?>
                    <div class="col-md-4">
                        <div class="card shadow-sm p-3 border-start border-4 border-success">
                            <h5 class="fw-bold"><?php echo htmlspecialchars($row['title']); ?></h5>
                            <p class="small text-muted mb-2">📅 <?php echo date('d M Y, h:i A', strtotime($row['event_start'])); ?></p>
                            <div class="d-grid gap-2">
                                <a href="manage_event.php?id=<?php echo $row['id']; ?>" class="btn btn-outline-dark btn-sm">⚙️ Manage & Guards</a>
                            </div>
                        </div>
                    </div>
                    <?php endwhile; ?>
                <?php else: ?>
                    <div class="text-center py-5"><p class="text-muted">You haven't created any events.</p></div>
                <?php endif; ?>
            </div>
        </div>
                    ...
                    <div class="col-md-4">
                        <div class="card shadow-sm h-100">
                            <div class="card-body">
                                ...
                                <h6 class="card-subtitle mb-2 text-muted small"><?php echo htmlspecialchars($row['venue']); ?></h6>
                                <a href="event_details.php?id=<?php echo $row['id']; ?>" class="btn btn-primary w-100 mt-2">Details & Register</a>
                            </div>
                        </div>
                    </div>
                    <?php endwhile; ?>
...
<footer class="text-center py-4 mt-5" style="border-top: 1px solid #e9ecef;">
    <div class="container">
        <p class="text-muted small mb-1">&copy; <?php echo date('Y'); ?> Hackathon Hub</p>
        <a href="guard_login.php" class="text-decoration-none text-secondary fw-bold" style="font-size: 13px;">🔒 Staff / Guard Access</a>
    </div>
</footer>

<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>

<div id="emergency-overlay" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(220, 53, 69, 0.95); z-index: 10000; align-items: center; justify-content: center; text-align: center; color: white;">
    <div style="background: white; color: black; padding: 30px; border-radius: 15px; width: 90%; max-width: 500px;">
        ...
        <button onclick="document.getElementById('emergency-overlay').style.display='none'" class="btn btn-dark w-100">Close</button>
    </div>
</div>
...
let myEventIds = [
    <?php 
    $check_ids = $conn->query("SELECT hackathon_id FROM registrations WHERE user_id = $user_id");
    $ids = [];
    if($check_ids) { while($r = $check_ids->fetch_assoc()) { $ids[] = $r['hackathon_id']; } }
    echo implode(',', $ids);
    ?>
];
let lastMsg = "";
