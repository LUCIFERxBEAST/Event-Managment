...
// 2. Check Ownership
$check_sql = "SELECT * FROM hackathons WHERE id = $event_id AND created_by = $user_id";
$event_result = $conn->query($check_sql);
...
<head>
    ...
    <style>
        .status-present { color: #0f5132; background-color: #d1e7dd; padding: 4px 8px; border-radius: 6px; font-weight: bold; }
        .status-reg { color: #856404; background-color: #fff3cd; padding: 4px 8px; border-radius: 6px; }
    </style>
</head>
<body class="bg-light">
...
<div class="container my-5">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2>📊 Participants</h2>
            <h5 class="text-muted"><?php echo htmlspecialchars($event['title']); ?></h5>
        </div>
        <div>
            <a href="manage_event.php?id=<?php echo $event_id; ?>" class="btn btn-outline-secondary me-2">← Back</a>
            <a href="api/export_participants.php?event_id=<?php echo $event_id; ?>" class="btn btn-success fw-bold">📥 Download Excel</a>
        </div>
    </div>
